...
A comprehensive guide to creating extraordinary software by orchestrating swarms of AI agents using exhaustive markdown plans,
polished beads, and the Agent Flywheel stack. Based on the methodology of Jeffrey Emanuel.
...
You explain what you want to build to a frontier model like GPT Pro with Extended Reasoning. Your concept, your goals, the user workflows, why it matters. The model produces an initial markdown plan: a comprehensive design document for the entire system.
You ask competing models to create their own plans. Claude Opus, Gemini with Deep Think, Grok Heavy. Each one independently designs the same project. They come up with surprisingly different approaches, each with unique strengths and blind spots.
You synthesize the best ideas from all plans into one. GPT Pro analyzes the competing plans and produces a "best of all worlds" hybrid that blends the strongest ideas from every model into a single superior document.
You iterate relentlessly. Round after round of refinement, each time in a fresh conversation, until the suggestions become incremental. Plans created this way routinely reach 3,000 to 6,000+ lines. They are not slop. They are the result of countless iterations and feedback from many frontier models.
You convert the plan into beads. Beads are self-contained work units (like Jira or Linear tasks, but optimized for use by coding agents). Each bead carries its own context, reasoning, dependencies, and test obligations. A complex plan might produce 200-500 beads with a full dependency graph.
You polish the beads obsessively. "Check your beads N times, implement once," where N is as many as you can stomach. Each polishing round finds things the previous round missed: duplicates, missing dependencies, incomplete context. You run this 4-6+ times until convergence.
You launch a swarm of agents. Claude Code, Codex, and Gemini-CLI sessions running in parallel, all in the same codebase. They coordinate through Agent Mail, choose work intelligently using bv's graph-theory routing, and execute beads systematically.
You tend the swarm, not the code. The human checks for stuck beads, rescues agents after context compaction, sends review prompts, and ensures flow quality. You are the clockwork deity. You designed the machine, set it running, and now you manage it.
Agents review, test, and harden. Self-review with fresh eyes, cross-agent review, random code exploration, testing coverage, UI/UX polish. Rounds and rounds until reviews come back clean.
...
5,500
LINE PLAN
Synthesized from 4 frontier models 347
BEADS
With full dependency structure
11k
LINES OF CODE
Working, tested, production-ready ~5 hrs
TO SHIP
25 agents, 204 commits Once you have the beads in good shape based on a great markdown plan, I almost view the project as a foregone conclusion at that point. The rest is basically mindless "machine tending" of your swarm of 5-15 agents.
...
TERM
Markdown plan
PLAIN-ENGLISH MEANING A huge design document where the whole project still fits in context
WHY IT MATTERS
Where architecture, workflows, tradeoffs, and intent get worked out TERM
Bead
PLAIN-ENGLISH MEANING A self-contained work unit in br with context, dependencies, and test obligations
WHY IT MATTERS
What agents actually execute TERM
Bead graph
PLAIN-ENGLISH MEANING The full dependency structure across all beads
WHY IT MATTERS
What lets bv compute the right next work TERM
Plan space
PLAIN-ENGLISH MEANING The reasoning mode where you are still shaping the whole system
WHY IT MATTERS
The cheapest place to buy correctness TERM
Bead space
PLAIN-ENGLISH MEANING The reasoning mode where you are shaping executable work packets
WHY IT MATTERS
Where planning becomes swarm-ready TERM
Code space
PLAIN-ENGLISH MEANING The implementation and verification layer inside the codebase
WHY IT MATTERS
Where local execution happens TERM
AGENTS.md
PLAIN-ENGLISH MEANING The operating manual every agent must reload after compaction
WHY IT MATTERS
Keeps the swarm from forgetting how to behave TERM
Skill
PLAIN-ENGLISH MEANING A reusable instruction bundle that teaches agents how to use a tool or execute a workflow
WHY IT MATTERS
How methods become repeatable instead of staying as tacit lore TERM
Compaction
PLAIN-ENGLISH MEANING Context compression inside a long-running agent session
WHY IT MATTERS
Why re-reading AGENTS.md is mandatory TERM
Fungible agents
PLAIN-ENGLISH MEANING Generalist agents that can replace one another
WHY IT MATTERS
...
INTEGRATE SYNTHESIS REVISIONS OK, now integrate these revisions to the markdown plan in-place; use ultrathink and be meticulous. At the end, you can tell me which changes you wholeheartedly agree with, which you somewhat agree with, and which you disagree with:
...
GPT Pro
INCLUDED
System-wide coherence Best-of-all-worlds synthesis
IMPLEMENTATION REALIST Claude Opus
INCLUDED
Execution detail
Sharp structural edits COVERAGE EXPANDER
Gemini
INCLUDED
Alternative framings
Missed edge cases
ASSUMPTION STRESS-TEST Grok Heavy
INCLUDED
Counterintuitive options Pressure-testing assumptions
...
PLAN REFINEMENT PROMPT Carefully review this entire plan for me and come up with your best revisions in terms of better architecture, new features, changed features, etc. to make it better, more robust/reliable, more performant, more compelling/useful, etc.
...
CRITICAL
Watch for the plan-bead gap. In the destructive_command_guard sessions, agents occasionally got stuck between plan revision and bead creation: the competing plan was revised but no beads were ever created. Always ensure the synthesis step concludes with an explicit transition to bead creation.
...
CRITICAL
Never write pseudo-beads in markdown documents. Go directly from the markdown plan to actual real beads using the br tool, and from that point on just add and change actual beads. If the model starts describing beads in text form instead of creating them, stop it and redirect to br create.
...
CARRIES CONCEPT
OUTCOME
Validate uploads, chunk large files, parse them, and emit ingestion telemetry.
ACCEPTANCE
Reject malware, cap retries, persist parse status, and record timing for every stage.
TESTS
Fixture matrix for malformed PDFs, retry exhaustion, and upload resume edge cases.
BR-102
Search Index and Query UX DEPS: BR-101
CARRIES CONCEPT
OUTCOME
...
BR-103
Ingestion Failure Dashboard DEPS: BR-101
OUTCOME
...
Ready
Ship
Dependencies Stabilizing Content Similarity Rising
Length Delta Approaching Zero Semantic Density Plateau
29.0%
...
1-3
CHARACTER
Wild swings, fundamental changes PHASE
Architecture
ROUNDS
4-7
CHARACTER
Interface improvements, boundary refinements PHASE
Refinement
ROUNDS
8-12
CHARACTER
Edge cases, nuanced handling PHASE
Polishing
ROUNDS
13+
CHARACTER
...
CRITICAL
Early termination red flags: If you see oscillation (alternating between two versions), reframe the problem. If you see expansion (output growing instead of shrinking), step back; the agent is adding complexity. If you see a plateau at low quality, kill the current approach and restart fresh.
...
FRESH REVIEW OF BEADS We recently transformed a markdown plan file into a bunch of new beads. I want you to very carefully review and analyze these using `br` and `bv`. Check over each bead super carefully-- are you sure it makes sense? Is it optimal? Could we change anything to make the system work better for users? If so, revise the beads. It's a lot easier and faster to operate in "plan space" before we start implementing these things! Use ultrathink.
...
Ground in reality. Read AGENTS.md and list all existing beads (br list --json). This prevents creating duplicates.
Generate 30, winnow to 5. The agent brainstorms 30 ideas for improvements, then self-selects the best 5 with justification.
Expand to 15. Prompt: "ok and your next best 10 and why." The agent produces ideas 6-15, checking each against existing beads for novelty.
Human review. You review the 15 ideas and select which to pursue.
Turn into beads. Selected ideas become beads with full descriptions, dependencies, and priority levels.
Refine 4-5 times. The same polishing loop as above. Single-pass beads are never optimal.
IDEA-WIZARD: GENERATE IDEAS Come up with 30 ideas for improvements, enhancements, new features, or fixes for this project. Then winnow to your VERY best 5 and explain why each is valuable.
...
STEP 1: INVESTIGATE AND PROPOSE I want you to clone https://github.com/<external-project> to tmp and then investigate it and look for useful ideas that we can take from that and reimagine in highly accretive ways on top of existing <your project> primitives that really leverage the special, innovative concepts and value-add from both projects to make something truly special and radically innovative. Write up a proposal document, PROPOSAL_TO_INTEGRATE_IDEAS_FROM_<EXTERNAL>_INTO_<YOUR_PROJECT>.md
...
STEP 4: REPEATED BLUNDER HUNT (RUN 5X) Look over everything in the proposal for blunders, mistakes, misconceptions, logical flaws, errors of omission, oversights, sloppy thinking, etc.
...
STEP 5: CLOSE DESIGN GAPS OK so then add this stuff to the proposal, using the very smartest ideas from your alien skills to inform it and your best judgment based on the very latest and smartest academic research:
- <specific design gap identified in blunder hunt> - <another specific gap>
...
STEP 7: MULTI-MODEL FEEDBACK How can we improve this proposal to make it smarter and better-- to make the most radically innovative and accretive and useful and compelling additions and revisions you can possibly imagine. Give me your proposed changes in the form of git-diff style changes against the file below, which is named PROPOSAL_TO_INTEGRATE_IDEAS_FROM_<EXTERNAL>_INTO_<YOUR_PROJECT>.md:
...
STEP
Investigate and propose PURPOSE
Ground the proposal in real study of the external project REPETITIONS
STEP
Iterative deepening
PURPOSE
Push past conservative initial suggestions REPETITIONS
1-2
STEP
Inversion analysis
PURPOSE
Find opportunities unique to your project's capabilities REPETITIONS
STEP
5x blunder hunt
PURPOSE
Ruthlessly verify after each expansion REPETITIONS
5x after each major step STEP
Architectural innovation PURPOSE
Push for specific novel design choices REPETITIONS
As needed
STEP
Close design gaps
PURPOSE
Convert honest unknowns into concrete decisions REPETITIONS
As needed
STEP
Self-contained background PURPOSE
Prepare for cross-model review REPETITIONS
STEP
Multi-model feedback + synthesis PURPOSE
Triangulate from 4 frontier models REPETITIONS
STEP
Apply diffs + de-slopify PURPOSE
Produce the final integrated proposal REPETITIONS
...
No git worktrees. Worktrees demolish development velocity and create reconciliation debt when agents diverge. Working in one shared space surfaces conflicts immediately. All agents commit directly to main.
Advisory file reservations. Agents call dibs temporarily on files, but it's not rigidly enforced, and reservations expire. Agents can reclaim files that haven't been touched recently. Rigid locks held by dead agents block everyone else. Advisory reservations with TTL expiry degrade gracefully.
...
Rule 0, The Override Prerogative: The human's instructions override everything.
Rule 1, No File Deletion: Never delete files without explicit permission.
No destructive git commands: git reset --hard, git clean -fd, rm -rf are absolutely forbidden.
Branch policy: All work happens on main, never master.
No script-based code changes: Always make code changes manually.
No file proliferation: No mainV2.rs or main_improved.rs variants.
Compiler checks after changes: Always verify no errors were introduced.
Multi-agent awareness: Never stash, revert, or overwrite other agents' changes.
...
POST-COMPACTION RESET Reread AGENTS.md so it's still fresh in your mind.
...
NOTE
DCG origin story: On December 17, 2025, an agent ran git checkout -- on uncommitted work. Files were recovered via git fsck --lost-found, but the incident proved that instructions do not prevent execution. Mechanical enforcement does. DCG was built the next day.
BLOCKED COMMAND
git reset --hard
SAFE ALTERNATIVE
git stash
...
Checks remote unchanged BLOCKED COMMAND
git clean -fd
SAFE ALTERNATIVE
git clean -fdn (preview first)
WHY
Shows what would delete BLOCKED COMMAND
rm -rf /path
SAFE ALTERNATIVE
rm -ri /path
WHY
...
> *Interactive version available in the original web interface.*
         ...
         [orange] AmberFox recovered orphaned br-104: Auth model.
         [orange] AmberFox recovered orphaned br-102: Search index.
       Footer text: "3 agents killed, 3 beads recovered. The swarm continues.
       No bottlenecks, no single points of failure. Like RaptorQ fountain codes:
       any agent catches any bead in any order."
     
     RESET SWARM button restores all agents to default state.
-->
...
ScarletCave
br-101: Upload pipeline BlueLake
br-102: Search index
CoralBadger
br-103: Admin dashboard JadePeak
br-104: Auth model
AmberFox
br-105: E2E tests
IvoryOwl
br-106: API layer
EVENT LOG
Swarm initialized. All agents working.
...
The beads themselves contain detailed descriptions, context, and rationale embedded during the plan-to-bead conversion.
AGENTS.md contains project-specific rules, conventions, and tool documentation.
The codebase contains the actual implementation context.
...
Run agents in separate tabs using WezTerm and its built-in mux, often across remote machines Trigger your most common prompts from a Stream Deck with the prompts preconfigured
Keep a large prompt file open in Zed and paste rarer prompts manually In Claude Code, use the project-specific Ctrl-r prompt history search when you want to recall something you used recently
...
WezTerm persistent remote sessions: its native mux supports persistent remote sessions that survive disconnects, sleep, or reboot while preserving native scrollback and text selection Ghostty terminfo for remote machines: Ghostty is a good terminal frontend in its own right, whether used directly or paired with another mux such as Zellij
...
SWARM MARCHING ORDERS First read ALL of the AGENTS.md file and README.md file super carefully and understand ALL of both! Then use your code investigation agent mode to fully understand the code, and technical architecture and purpose of the project. Then register with MCP Agent Mail and introduce yourself to the other agents.
...
Agent Composition & Model Recommendations PHASE
Initial plan creation RECOMMENDED MODEL
GPT Pro (web)
WHY
Extended reasoning, all-you-can-eat pricing PHASE
Plan synthesis
RECOMMENDED MODEL
GPT Pro (web)
WHY
Best at being the 'final arbiter' PHASE
Plan refinement
RECOMMENDED MODEL
GPT Pro + Opus (web)
WHY
Pro reviews, Claude integrates PHASE
Plan → Beads conversion RECOMMENDED MODEL
Claude Code (Opus)
WHY
Best coding agent for structured creation PHASE
Bead polishing
RECOMMENDED MODEL
Claude Code (Opus)
WHY
...
Claude Code + Gemini
WHY
Gemini good for review duty PHASE
Final verification
RECOMMENDED MODEL
Codex (GPT)
WHY
Different model catches different things
...
OPEN BEADS	CLAUDE (CC)	CODEX (COD)	GEMINI (GMI) 400+	4	4	2
100-399	3	3	2
<100	1	1	1
The Thundering Herd
...
Who starts when
Claim
First contact with ready beads Resolve
Lock collisions or clean frontier Flow
Parallel work settles in Outcome
Who spent the cycle shipping Thundering Herd
...
br-203
br-204
Nothing is broken yet, but the timing guarantees a crowded first claim.
Staggered Start
...
Check bead progress. Use br list --status in_progress --json or bv --robot-triage. Are agents making steady progress? Are any beads stuck? Handle compactions. When you see an agent acting confused, send: "Reread AGENTS.md so it's still fresh in your mind." This is the single most common intervention. It takes 5 seconds.
Run periodic reviews. Pick an agent and send the "fresh eyes" review prompt. This catches bugs before they compound.
Manage rate limits. When an agent gets rate-limited, switch its account with caam activate claude backup-2 or start a new agent.
Commit periodically. Every 1-2 hours, designate one agent for the organized commit prompt.
Handle surprises. Create new beads for unanticipated issues, or if it's plan-level, update the plan and create new beads.
...
SYMPTOM
Multiple agents keep picking the same bead LIKELY CAUSE
Starts were not staggered; agents are not marking in_progress WHAT TO DO
Stagger starts, force explicit Agent Mail claim messages, check reservations SYMPTOM
Agent goes in circles after compaction LIKELY CAUSE
It forgot the operating contract in AGENTS.md WHAT TO DO
Force Reread AGENTS.md; kill/restart the session if still erratic SYMPTOM
A bead sits in_progress for too long LIKELY CAUSE
Agent crashed, silently blocked, or lost the plot WHAT TO DO
Check Agent Mail, reclaim the bead, split out the blocker into a clearer bead SYMPTOM
Agents produce contradictory implementations LIKELY CAUSE
Not coordinating through Agent Mail and reservations WHAT TO DO
Audit reservation use, revise bead boundaries if overlapping SYMPTOM
Lots of code and commits, but goal still feels far LIKELY CAUSE
Strategic drift; current beads do not close the remaining gap WHAT TO DO
Stop, run the reality check prompt, revise bead graph
...
Is the implementation correct? Does it do what the bead description says it should? Are there edge cases? Empty inputs, concurrent access, error paths, boundary conditions.
Are there similar issues elsewhere? If you find a bug, search for the same pattern in other files.
Should the approach be different? Sometimes the implementation is correct but there is a simpler or more robust way.
...
Emdash overuse
PROBLEM
LLMs use emdashes constantly, even when semicolons, commas, or sentence splits work better PATTERN
"It's not X, it's Y"
PROBLEM
Formulaic contrast structure PATTERN
"Here's why" / "Here's why it matters:" PROBLEM
Clickbait-style lead-in PATTERN
"Let's dive in"
PROBLEM
Forced enthusiasm
PATTERN
"At its core..."
PROBLEM
Pseudo-profound opener PATTERN
"It's worth noting..."
PROBLEM
Unnecessary hedge
...
RANDOM CODE EXPLORATION I want you to sort of randomly explore the code files in this project, choosing code files to deeply investigate and understand and trace their functionality and execution flows through the related code files which they import or which they are imported by.
...
HIGH-LEVEL REALITY CHECK Where are we on this project? Do we actually have the thing we are trying to build? If not, what is blocking us? If we intelligently implement all open and in-progress beads, would we close that gap completely? Why or why not?
...
QUICK FINAL SANITY CHECK Great. Look over everything again for any obvious oversights or omissions or mistakes, conceptual errors, blunders, etc.
...
Sync beads. br sync --flush-only to export to JSONL, then git add .beads/.
Commit and push. git pull --rebase && git add <files> && git commit && git push.
...
ntm
PURPOSE
Named Tmux Manager — agent cockpit for spawning, sending, and broadcasting TOOL
COMMAND
am
PURPOSE
Agent coordination — identities, inbox/outbox, file reservations TOOL
UBS
COMMAND
ubs
PURPOSE
Ultimate Bug Scanner — 1000+ patterns, pre-commit guardrails TOOL
Beads
COMMAND
PURPOSE
Issue tracking — dependency-aware, JSONL+SQLite hybrid TOOL
Beads Viewer
COMMAND
PURPOSE
Triage engine — PageRank, betweenness, HITS, robot mode TOOL
RCH
COMMAND
rch
PURPOSE
Remote build offloading — keeps heavy CPU work off the swarm box TOOL
CASS
COMMAND
cass
PURPOSE
Session search — unified agent history indexing TOOL
CASS Memory
COMMAND
cm
PURPOSE
Procedural memory — episodic → working → procedural TOOL
CAAM
COMMAND
caam
PURPOSE
Auth switching — sub-100ms account swap across providers TOOL
DCG
COMMAND
dcg
PURPOSE
Safety guard — blocks destructive git/filesystem operations mechanically TOOL
SLB
COMMAND
slb
PURPOSE
Two-person rule — optional guardrails for dangerous commands
...
Shell
VALUE
zsh (with oh-my-zsh + powerlevel10k) ASPECT
Workspace
VALUE
/data/projects
ASPECT
...
Start with: Agent Mail + Beads (br) + Beads Viewer (bv) — this core trio captures most of the value Then add: UBS for bug hunting
Then add: DCG for destructive command protection Then add: CASS for session history
Then add: CM (CASS Memory) for codifying lessons into procedural memory CRITICAL
Common beginner mistakes: Making a hasty plan all at once instead of the multi-model iterative process. Trying to convert the plan to beads in a single pass. Not doing at least 3 rounds of polishing. "Well, of course the project is going to suck and be a buggy mess if you do that."

### Scale Observations from Real Projects

PROJECT	BEADS	PLAN LINES	AGENTS	TIME TO MVP CASS Memory System	347+	5,500	~25	~5 hours
FrankenSQLite	Hundreds	Large spec	Many parallel	Multi-session Frankensearch	122+ (3 epics)	—	Multiple	Multi-session
...
AGENTS.md
PURPOSE
Per-project configuration teaching agents about tools and rules COMPONENT
Best practices guides PURPOSE
Referenced in AGENTS.md, kept current COMPONENT
Markdown plan files
PURPOSE
Source-of-truth planning documents COMPONENT
acfs newproj
PURPOSE
Bootstraps projects with full tooling (.git, .beads, .claude, AGENTS.md) COMPONENT
acfs doctor
PURPOSE
Single command to verify entire installation COMPONENT
NTM command palette
PURPOSE
Battle-tested prompt library accessible via ntm palette COMPONENT
Claude Code Skills
PURPOSE
Each tool has a dedicated skill for automated workflows
...
GATE
Foundation
MUST BE TRUE BEFORE ADVANCING Goals, workflows, stack, architecture direction, AGENTS.md, and best-practices guides exist and are coherent
GATE
Plan
MUST BE TRUE BEFORE ADVANCING Markdown plan covers workflows, architecture, sequencing, constraints, testing expectations, and major failure paths
GATE
Translation
MUST BE TRUE BEFORE ADVANCING Every material plan element maps to one or more beads, checked in both directions
GATE
Bead
MUST BE TRUE BEFORE ADVANCING Beads are self-contained, dependency-correct, rich in context, and explicit about test obligations
GATE
Launch
MUST BE TRUE BEFORE ADVANCING Agent Mail, file reservations, bead IDs, bv, AGENTS.md, and staggered startup are all ready
GATE
Ship
MUST BE TRUE BEFORE ADVANCING Reviews, tests, UBS, remaining-work beads, and feedback capture into reusable artifacts are complete
...
SKILL REFINEMENT META-SKILL Search CASS for all sessions where agents used the [SKILL] skill. Look for: patterns of confusion, repeated mistakes, steps agents skipped, workarounds they invented, and things they did that weren't in the skill but should be.
...
RITUAL
ACTION
Validated methodology. Extract into a skill.
REPETITION COUNT
count 5-9
STATUS
Emerging pattern
...
Global reasoning belongs in plan space. Do the hardest architectural and product reasoning while the whole project still fits in context.
The markdown plan must be comprehensive before coding starts. Skeleton-first coding throws away the main advantage of frontier models.
Plan-to-beads is a distinct translation problem. A good plan does not automatically produce a good bead graph.
Beads are the execution substrate. Once good enough, they should carry enough context that agents no longer need the full plan.
Convergence matters more than first drafts. Plans and beads both improve through repeated polishing until changes become small and corrective.
Swarm agents are fungible. Coordination must live in artifacts and tools, not in special agents or unstated knowledge.
Coordination must survive crashes and compaction. AGENTS.md, Agent Mail, bead state, and robot modes exist to keep work moving when sessions die.
Session history is part of the system. Repeated prompts, failures, and recoveries should be mined via CASS and folded back into tools, skills, and validators.
Implementation is not the finish line. Review, testing, UBS, and feedback-to-infrastructure loops are part of the core method.

### Time Investment

PHASE
Planning (multi-model synthesis + refinement) TYPICAL DURATION
3+ hours for a complex feature NOTES
Feels slow because no code is written, but the downstream payoff is enormous PHASE
Plan to beads conversion TYPICAL DURATION
Significant; Claude needed 'coaxing and cajoling' for 347 beads NOTES
A 5,500-line plan with hundreds of beads takes sustained effort PHASE
Bead polishing
TYPICAL DURATION
2-3 passes per session, multiple sessions NOTES
Agents typically manage 2-3 passes before running out of context window PHASE
Implementation with swarm TYPICAL DURATION
Remarkably fast
NOTES
CASS Memory System: 11k lines in ~5 hours with 25 agents
...
Build the baseline. Create a tool. Create a skill for it using sc (skill creator). The first version of the skill will be imperfect. Ship it anyway.
Let agents use it in real work. Do not test in isolation. Deploy the tool and skill into actual project work where agents are implementing beads, running reviews, doing real tasks. CASS automatically logs every session.
...
Clarifying questions: where agents asked "do you mean X or Y?" means the skill was ambiguous Repeated mistakes across different agents: a systematic gap in the skill's instructions
Creative workarounds: agents inventing their own approach means the skill is missing a useful pattern Outright failures: the skill directed agents to do something wrong or impossible
...
CASS-Powered Refinement Skills That Generate Work
Skills Bundled With Installers

Layer 1 requires zero infrastructure. You can do this today, right now, with any tool and two agent sessions.

Layer 1: Feedback forms after tool use (start here, no infrastructure needed). After an agent finishes using a tool, ask it to fill out a structured feedback survey. Feed that to another agent working on the tool itself. This requires nothing beyond two agent sessions and produces immediate improvements.
Layer 2: CASS-powered skill refinement (requires session logging). Instead of relying on one agent's opinion, mine session logs to find systematic patterns across many agents. An agent using a tool for the first time might blame itself for a confusing flag; when you see 15 agents all struggling with the same flag, you know the flag is the problem.
...
Layer 4: Skills bundled with tool installers (the skill improves before the user ever sees it). Every tool you ship includes a pre-optimized Claude Code skill baked into its installer. The skill was refined through multiple CASS cycles before shipping. When a new user installs the tool, their agents immediately benefit from all the refinement work done across every previous user's sessions.
...
OPERATOR 1: PLAN-FIRST EXPANSION [OPERATOR: plan-first-expansion]
...
OPERATOR 2: COMPETING-PLAN TRIANGULATION [OPERATOR: competing-plan-triangulation]
...
OPERATOR 3: OVERSHOOT MISMATCH HUNT [OPERATOR: overshoot-mismatch-hunt]
...
OPERATOR 4: PLAN-TO-BEADS TRANSFER AUDIT [OPERATOR: plan-to-beads-transfer-audit]
...
OPERATOR 5: CONVERGENCE POLISH LOOP [OPERATOR: convergence-polish-loop]
...
OPERATOR 6: FRESH-EYES RESET [OPERATOR: fresh-eyes-reset]
...
OPERATOR 7: FUNGIBLE SWARM LAUNCH [OPERATOR: fungible-swarm-launch]
...
OPERATOR 8: FEEDBACK-TO-INFRASTRUCTURE CLOSURE [OPERATOR: feedback-to-infrastructure-closure]
